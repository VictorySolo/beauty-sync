...
const RelatedSpecialists = ({speciality, specialistId}) => {

  const navigate = useNavigate()
  const {beautySpecialists} = useContext(AppContext)

  const [relSpec, setRelSpec] = useState([])


  useEffect(()=>{
    if (beautySpecialists.length > 0 && speciality) {
      const specData = beautySpecialists.filter((spec) => spec.speciality === speciality && spec._id !== specialistId)
      setRelSpec(specData)
    }
  }, [beautySpecialists, speciality, specialistId])
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-zinc-300 md:mx-10'>
       ...
       <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {relSpec.slice(0,5).map((item,index)=>(
